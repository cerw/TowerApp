...
function activityStatus() {
  return (
    ...
    global.motion +
    " Touch:" +
    global.touchactivity +
    " Keys:" +
    global.keyactivity +
    " Alive:" +
    global.alive +
    " Lights: " +
    global.lightactivity
  );
}
...
function activityCheck(rerun = true) {
  if (
    (global.motion || global.touchactivity || global.keyactivity) &&
    !global.alive
  ) {
    // recent motion
    global.alive = 1;
...
mqtt.on("message", function (topic, message) {
  ...
  if (topic === "motion") {
    ...
    if (json.e === "start") {
      // activityCheck(false)
      // system.say('Motion strated')
      // screen.mqttlog('Motind strated', json)
      if (!global.alive) {
